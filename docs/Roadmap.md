...
**v1.5.6 预算识别修复与 UI 优化 — 已完成** ✅

下一阶段：**v1.6 — 待定**

## v1.5.6 预算识别修复与 UI 优化 ✅

### 预算识别修复 ✅
- [x] 删除 `"清单"` 关键词，避免非预算文件误判
- [x] stem_no_digits prefix 匹配（解决点位名含后缀描述时的漏判）
- [x] `_score_file_to_point` 4 层分层评分（Tier 1~4）
- [x] 反向排斥逻辑（stem 含其他点位名或分纤箱扩容）
- [x] `unassigned_budget_files` 字段标记未知点位预算文件
- [x] `build_organize_plan` 避免「设计文件」目录嵌套

### UI 优化 ✅
- [x] 「扫描结果中心」→「扫描中心」
- [x] 「文件整理预览」+「执行整理」合并为「整理文件」
- [x] 扫描中心项目选择下拉框
- [x] 窗口横向缩放修复
- [x] 项目选择器独立横幅布局
...
### 归属证据 ✅
- [x] 扫描递归遍历用户选择目录下所有文件
- [x] 归属判断使用文件名/父目录/祖先目录片段/项目相对路径
- [x] 泛分类目录名不作为点位身份证据
- [x] 图纸严格证据保留，禁止 fuzzy，避免跨点位污染回归
- [x] 预算 PDF 可按预算资料归属并最终分类为预算

### 建议文案与整理闭环 ✅
- [x] NOT_FOUND 建议不再误提示创建点位文件夹
- [x] 有归属文件时建议执行整理创建标准目录
- [x] 整理预览/执行复用 Scan Session，不重新扫描
- [x] 整理目标目录使用 scan_path，并生成 点位名/图纸|预算|其他文件

## v1.5.4 点位明细导入去重 ✅

### 任务名称去重 ✅
- [x] 点位明细/规模表导入时按标准化点位/任务名称去重
- [x] 同名任务只导入一次，避免 point_dictionary 重复点位
- [x] 重复行用于补充首条记录中的空区县和空动态字段
- [x] 导入进度和完成提示显示重复跳过数量
- [x] 预览阶段提示导入会自动去重

## v1.5.3 扫描结果生命周期管理（Scan Session Lifecycle）✅

### Scan Session ✅
- [x] 一次扫描完成后生成当前项目 Scan Session
- [x] 保存 project_id / scan_path / scan_time / 文件索引 / ownership归属结果 / ScanResult
- [x] 重新扫描时替换旧 Scan Session
- [x] 修改扫描目录时使旧 Scan Session 失效

### 禁止重复扫描 ✅
- [x] 文件整理预览读取当前 Scan Session，禁止内部再次调用 scanner
- [x] 文件整理执行读取当前 Scan Session，禁止内部再次调用 scanner
- [x] 点位详情只读缓存结果，不触发扫描
- [x] 分类统计维持只读缓存结果，无缓存时不扫描

### 按钮逻辑 ✅
- [x] 初次显示「执行扫描」
- [x] 有有效 Scan Session 后显示「重新扫描」
- [x] 无扫描结果时文件整理预览提示「请先执行扫描」
